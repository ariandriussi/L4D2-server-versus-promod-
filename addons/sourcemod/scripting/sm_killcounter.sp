...
public OnPluginStart()
{
	...
	friendly = CreateConVar("sm_killcounter_f", "1", "Friendly Fire message. 0: Off 1: On",FCVAR_NONE
,true,0.0,true,1.0);
...
{
	PrintToChatAll("\x03 gracias por entrar al servidor   ", PLUGIN_PREFIX);
}
...
void:Void_PrintKillInfo(attacker, bool:g_bHeadshot)
{
	...
	switch(g_iMode)
	{
		case 1:
		{
			...
			{
				if(g_iTemp >= 1)
					PrintHintText(attacker, " ", g_iTemp);
				else
...
public Action:Command_Kills(client, args)
{
	...
	Format(g_sTemp, sizeof(g_sTemp), "Headshot Kills: %d", g_iHeadshots);
	DrawPanelText(g_hPanel, g_sTemp);
	
	Format(g_sTemp, sizeof(g_sTemp), "Normal Kills: %d", g_iZombies);
	DrawPanelText(g_hPanel, g_sTemp);

	Format(g_sTemp, sizeof(g_sTemp), "Total Kills: %d", g_iKills);
	DrawPanelText(g_hPanel, g_sTemp);

	if(g_iKills)
	g_fPercent = 100 * g_iHeadshots / g_iKills;
	Format(g_sTemp, sizeof(g_sTemp), "Headshot Percentage: %d %6", g_fPercent, "%");
	DrawPanelText(g_hPanel, g_sTemp);
	DrawPanelText(g_hPanel, "-==-==-==-==-");
...
public Action:Command_TeamKills(client, args)
{
	...
	if(g_iTeam >= 2)
	{
		...
		new Handle:g_hPanel = CreatePanel();
		SetPanelTitle(g_hPanel, "MUERTES DEL EQUIPO");
		DrawPanelText(g_hPanel, "-==-==-==-==-");

		Format(g_sTemp, sizeof(g_sTemp), "Muertes: %d", g_iTotalZombies);
		DrawPanelText(g_hPanel, g_sTemp);
		
		Format(g_sTemp, sizeof(g_sTemp), "Headshots: %d", g_iTotalHeadshots);
		DrawPanelText(g_hPanel, g_sTemp);
		
		Format(g_sTemp, sizeof(g_sTemp), "Total: %d", g_iTotalKills);
		DrawPanelText(g_hPanel, g_sTemp);
		
		if(g_iTotalKills)
		g_fTotalPercent = 100 * g_iTotalHeadshots / g_iTotalKills;
		Format(g_sTemp, sizeof(g_sTemp), "Headshot Percentage: %d %6", g_fTotalPercent, "%");
		DrawPanelText(g_hPanel, g_sTemp);
		if(g_iCount > 0)
		{
		...
		DrawPanelText(g_hPanel, "-==-==-==-==-");
		DrawPanelItem(g_hPanel, "<<<< CERRAR PANEL");
